...
pub fn repeat(
    ...
    let count = library::read_as_i64(values.remove(0))? as usize;

    let value = values.remove(0);

    let values = std::iter::repeat(value).take(count).collect::<Vec<Value>>();
...
mod tests {
    ...
    fn returns_invalid_argument_count_error_when_incorrect_count_of_arguments_were_passed() {
        ...
        let code_vector = vec!["(list:repeat)", "(list:repeat 1)", "(list:repeat 1 2 3)"];
