...
pub fn register(
    ...
    let new_list = interpreter.vec_to_list(vec![path, name, modifier_keys]);
...
mod tests {
    ...
    fn allows_to_set_modifier_keys() {
        ...
        let pairs = vec![
            (
                r#"nil"#,
                r#"(keyboard:register "/dev/input/event1" "Keyboard 1" '("a" "b"))"#,
            ),
            (
                r#"(list '("/dev/input/event1" "Keyboard 1" ("a" "b")))"#,
                "registered-keyboards",
            ),
        ];
