...
fn read_registered_keyboards(
    ...
    for (index, registered_keyboard) in
        registered_keyboards.into_iter().enumerate()
    {
...
pub fn define_global_mapping(
    ...
    let mapping =
        library::read_as_string(interpreter, values.remove(0))?.clone(); // todo: fix
...
mod tests {
    ...
    fn defines_new_mappings_with_keyboard_identifiers() {
        ...
        let pairs = vec![
            ("global-map", "'()"),
            (
                "(keyboard:define-global-mapping \"0:LeftControl+0:b\" #(+ 1 2))",
                "nil",
            ),
            (
                "global-map",
    ...
    fn defines_new_mappings_with_keyboard_names() {
        ...
        let pairs = vec![
            ("global-map", "'()"),
            (
                "(keyboard:define-global-mapping \"first:LeftControl+first:b\" #(+ 1 2))",
                "nil",
            ),
            (
                "global-map",
