...
fn key_chords_to_list(interpreter: &mut Interpreter, key_chords: Vec<KeyChord>) -> Value {
...
fn read_registered_keyboards(
    ...
    let registered_keyboards = library::read_as_vector(interpreter, registered_keyboards)?;
    ...
    for (index, registered_keyboard) in registered_keyboards.into_iter().enumerate() {
        ...
        let keyboard_name = library::read_as_string(interpreter, part[1])?.clone();
...
pub fn define_global_mapping(
    ...
    for mapping_part in mapping.split(" ") {
        let key_chord = nia_events::str_to_key_chord(mapping_part, &keyboard_name_to_index)
            .map_err(|_| Error::invalid_argument_error("Invalid key chord."))?;
    ...
    let new_mapping = interpreter.vec_to_list(vec![values, function_id.into()]);
    library::add_value_to_root_list(interpreter, "global-map", new_mapping)?;
...
mod tests {
    ...
    fn defines_new_mappings_with_keyboard_names() {
        ...
        interpreter
            .execute_in_main_environment("(keyboard:register \"/dev/input/event0\" \"second\")")
            .unwrap();
        interpreter
            .execute_in_main_environment("(keyboard:register \"/dev/input/event1\" \"first\")")
            .unwrap();
