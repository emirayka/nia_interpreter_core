...
pub fn send_spawn(
    ...
    let text = library::read_as_string_id(
        interpreter,
        values.remove(0)
    )?;

    let spawn_symbol_value = interpreter.intern_symbol_value("spawn");
    let spawn = interpreter.vec_to_list(vec!(
        spawn_symbol_value,
        Value::String(text)
    ));
...
mod tests {
    ...
    fn adds_action_to_action_list() {
        ...
        let pairs = vec!(
            ("--actions", "'()"),
            ("(action:send-spawn \"first\") --actions", "'((spawn \"first\"))"),
            ("(action:send-spawn \"second\") --actions", "'((spawn \"second\") (spawn \"first\"))"),
        );
