...
pub fn send_mouse_move_by(
    ...
    let mut move_by_x = library::read_as_i64(
        interpreter,
        values.remove(0)
    )?;

    let mut move_by_y = library::read_as_i64(
        interpreter,
        values.remove(0)
    )?;
...
mod tests {
    ...
    fn adds_action_to_action_list() {
        ...
        let pairs = vec!(
            ("--actions", "'()"),
            ("(action:send-mouse-move-by 2 3) --actions", "'((mouse-move-by 2 3))"),
            ("(action:send-mouse-move-by 3 4) --actions", "'((mouse-move-by 3 4) (mouse-move-by 2 3))"),
        );
