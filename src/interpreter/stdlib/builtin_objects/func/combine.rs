...
pub fn combine(
    ...
    for value in values {
        let argument_function_id = library::read_as_function_id(
            interpreter,
            value
        )?;

        let cons = interpreter.make_cons_value(
            Value::Function(argument_function_id),
            nil
        );
        arguments.push(cons)
    }
    ...
    let result = interpreter.make_cons_value(
    ...
    let constructed_function_id = interpreter.register_function(
...
mod tests {
    ...
    fn combines_functions() {
        ...
        let pairs = vec!(
            ...
            ("((func:combine #(+ %1 %2) (func:always 1) (func:always 2)))", "3"),
        );
