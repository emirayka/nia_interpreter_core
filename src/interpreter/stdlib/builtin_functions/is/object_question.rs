...
mod tests {
    ...
    #[test]
    fn returns_true_when_an_object_was_passed() {
        let mut interpreter = Interpreter::new();

        let expected = Value::Boolean(true);
        let result = interpreter.execute("(is:object? {})").unwrap();
    ...
    // todo: ensure this test is fine
    #[test]
    fn returns_false_when_not_an_object_was_passed() {
        for_value_pairs_evaluated_ifbsykcou(
            |interpreter, code, value| {
                ...
                let code = format!("(is:object? {})", code);
    ...
    fn returns_invalid_argument_count_error_when_incorrect_count_of_arguments_were_passed() {
        ...
        let result = interpreter.execute("(is:object?)");
        assertion::assert_invalid_argument_count_error(&result);

        let result = interpreter.execute("(is:object? 1 2)");
        assertion::assert_invalid_argument_count_error(&result);
    }
}
