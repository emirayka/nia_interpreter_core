...
mod tests {
    ...
    fn returns_true_when_an_function_was_passed() {
        ...
        let result = interpreter.execute("(is:function? #(+ %1 %2))").unwrap();
    ...
    fn returns_false_when_not_an_function_was_passed() {
        for_value_pairs_evaluated_ifbsykcou(
            |interpreter, code, value| {
                ...
                let code = format!("(is:function? {})", code);
    ...
    fn returns_invalid_argument_count_error_when_incorrect_count_of_arguments_were_passed() {
        ...
        let result = interpreter.execute("(is:function?)");
        assertion::assert_invalid_argument_count_error(&result);

        let result = interpreter.execute("(is:function? 1 2)");
        assertion::assert_invalid_argument_count_error(&result);
    }
}
