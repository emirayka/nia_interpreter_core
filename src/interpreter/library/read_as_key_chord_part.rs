...
pub fn read_as_key_chord_part(
    ...
    match key_chord_part_value {
        ...
        Value::Cons(cons_id) => {
            ...
            let keyboard_id = library::read_as_i64(values.remove(0))?;
            let key_id = library::read_as_i64(values.remove(0))?;
...
mod tests {
    ...
    fn assert_returns_correct_key_chord_part(
        ...
        let result = read_as_key_chord_part(&mut interpreter, list).unwrap();
