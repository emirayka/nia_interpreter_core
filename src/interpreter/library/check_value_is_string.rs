...
pub fn check_value_is_string(interpreter: &Interpreter, value: Value) -> Result<(), Error> {
    match value {
        Value::String(_) => Ok(()),
...
mod tests {
    ...
    fn returns_nothing_when_a_string_was_passed() {
        ...
        let result =
            check_value_is_string(&mut interpreter, Value::String(StringId::new(0))).unwrap();
    ...
    fn returns_invalid_argument_when_not_a_string_value_were_passed() {
        ...
        for not_string_value in not_string_values {
            let result = check_value_is_string(&mut interpreter, not_string_value);
