...
fn parse_key_id_from_vector(
    ...
    let key_id = match library::read_as_i64(
        interpreter,
...
fn parse_button_id_from_vector(
    ...
    let button_id = match library::read_as_i64(
        interpreter,
        event_vector.remove(0),
    ) {
        Ok(button_id) => {
            if button_id > 0 && button_id <= 8 {
                ...
                println!("Invalid button id, expected interval [1-8], got: {}", button_id);
                return Err(())
            }
        },
        Err(error) => {
...
fn send_events(
    ...
    let actions_value = match library::get_root_variable(
        interpreter,
        "--actions",
    ) {
    ...
    let events_vector = match library::read_as_vector(
        interpreter,
        actions_value,
    ) {
    ...
    match library::set_root_variable(
        interpreter,
        "--actions",
        nil,
    ) {
    ...
    for event in events_vector {
        let mut event_vector = match library::read_as_vector(
            interpreter,
            event,
        ) {
            Ok(vector) => vector,
        ...
        let event_name = match library::read_as_symbol_id(
            interpreter,
            event_vector.remove(0),
        ) {
            Ok(symbol_id) => {
                match interpreter.get_symbol_name(symbol_id) {
        ...
        let command = if event_name == "key-down" {
            ...
            let x = match library::read_as_i64(
                interpreter,
                event_vector.remove(0)
            ) {
                Ok(x) => x,
            ...
            let y = match library::read_as_i64(
                interpreter,
                event_vector.remove(0)
            ) {
                Ok(x) => x,
...
impl EventLoop {
    pub fn run_event_loop(
        ...
        let (
            cmd_sender,
            cmd_stopper
        ) = NiaCommandSender::new().start_sending();
