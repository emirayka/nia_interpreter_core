...
fn send_events(interpreter: &mut Interpreter, cmd_sender: &Sender<Command>) -> Result<(), ()> {
    let actions_value = match library::get_root_variable(interpreter, "--actions") {
    ...
    let events_vector = match library::read_as_vector(interpreter, actions_value) {
    ...
    for event in events_vector {
        ...
        let event_name = match library::read_as_symbol_id(event_vector.remove(0)) {
        ...
        let command = if event_name == "key-down" {
            let key_id = match parse_key_id_from_vector(interpreter, event_vector) {
                Ok(key_id) => key_id,
                Err(()) => continue,
            };
            ...
            let button_id = match parse_button_id_from_vector(interpreter, event_vector) {
                Ok(key_id) => key_id,
                Err(()) => continue,
            };
            ...
            Command::UinputCommand(UinputCommand::MouseMoveBy(x as i16, y as i16))
            ...
            if x < 0 || y < 0 {
                println!("Invalid move to event description. Expected two positive coordinates. Got: {}, {}", x, y);
                continue;
            }

            Command::UinputCommand(UinputCommand::MouseMoveTo(x as i16, y as i16))
            ...
            let text = match library::read_as_string(interpreter, event_vector.remove(0)) {
            ...
            let path = match library::read_as_string(interpreter, event_vector.remove(0)) {
                Ok(string) => string.clone(),
                Err(error) => {
                    println!("Cannot parse event cause:");
                    println!("{}", error);
            ...
            let milliseconds = match event_vector.remove(0) {
                Value::Integer(ms) => {
                    if ms >= 0 {
                        ...
                        println!("Expected duration to be not negative, got: {}", ms);
                        continue;
                    }
                }
                v => {
...
impl EventLoop {
    pub fn run_event_loop(
        ...
        let (command_sender, command_receiver) = mpsc::channel::<InterpreterCommand>();
        ...
        let mut event_listener_v: Option<(mpsc::Receiver<Action>, mpsc::Sender<()>)> = None;
        ...
        thread::spawn(move || {
            ...
            time_for_garbage_collection += Duration::from_millis(GARBAGE_COLLECTOR_PERIOD);
            ...
            loop {
                ...
                match command_receiver.try_recv() {
                    Ok(command) => match command {
                        InterpreterCommand::Execution(code) => {
                            ...
                            let execution_result = match result {
                                Ok(value) => {
                                    match library::value_to_string(&mut interpreter, value) {
                                        Ok(string) => ExecutionResult::Success(string),
                                        Err(error) => {
                                            if error.is_failure() {
                            ...
                            match result_sender
                                .send(CommandResult::ExecutionResult(execution_result))
                            {
                                Ok(()) => {}
                                Err(_) => break,
                            }
                ...
                if interpreter.is_listening() && event_listener_v.is_none() {
                    let mut event_listener = NiaEventListener::from_interpreter(&mut interpreter);
                ...
                send_events(&mut interpreter, &cmd_sender).expect("Sending events failed.");
                ...
                if current_time >= time_for_garbage_collection {
                    time_for_garbage_collection =
                        current_time + Duration::from_millis(GARBAGE_COLLECTOR_PERIOD);
